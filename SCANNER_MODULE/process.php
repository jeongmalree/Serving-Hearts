    ...
    if (!empty($userIdMatches[1])) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($booking['attendance_confirmed'] == 0) {
                ...
                echo json_encode(['status' => 'error', 'message' => 'Attendance already confirmed for this booking.']);
            }
        } else {
            // No pending booking found
            echo json_encode(['status' => 'error', 'message' => 'No pending booking found for this QR code.']);
        }
    } else {
