...
if (isset($_POST['signup-btn'])) {
    ...
    if (count($errors) === 0) {
        ...
        $sql = "INSERT INTO users (username, email, verified, token, verification_token_created_at, password, fullname, dateofbirth, gender, phonenumber, unique_number) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('ssbssssssss', $username, $email, $verified, $token, $verificationTokenCreatedAt, $password, $fullname, $dateofbirth, $gender, $phonenumber, $uniqueNumber);
...
if (isset($_POST['reset-password-btn'])) {
    ...
    if (count($errors) == 0) {
        ...
        if ($result) {
            ...
            // Redirect back to password_reset.php (the page where we want to auto-close)
            header('location: login.php');
            exit(0);
        }
    }
